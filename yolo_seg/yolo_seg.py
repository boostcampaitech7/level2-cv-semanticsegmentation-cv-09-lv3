...
def train(data_config_path: str):   
    ...
    model.train(
        ...
        # hsv_h=0.0,  # HSV-색조
        # hsv_s=0.0,    # HSV-채도
        # hsv_v=0.3,    # HSV-명도 (CLAHE와 유사한 효과)        
        # erasing=0.0,
        # mosaic=0.0,
        # shear=0.0,
        # perspective=0.0,
...
def inference():
    model = YOLO(f"runs/segment/train12/weights/best.pt").cuda()
    infer_images = sorted(glob("/home/work/.hat/dhlim/seg/level2-cv-ref/data/test/*/*/*.png"))
    rles = []
    filename_and_class = []

    for idx, infer_image in tqdm(enumerate(infer_images)):
        result = model.predict(infer_image, imgsz=2048, conf = 0.1, iou = 0.5)[0]
    ...
    if not os.path.exists('./result'):                                                           
        os.makedirs('./result')
    f_name = 'yolo_seg_augment2_2048_output.csv'
